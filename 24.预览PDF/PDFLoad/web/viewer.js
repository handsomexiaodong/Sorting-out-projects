  ...
  load: function load(pdfDocument, scale) {
    ...
    pdfDocument.getMetadata().then(function (_ref5) {
      ...
      console.log($("#viewer").height());
        console.log(pname);
        //$(".page").css("content",pname);
        //$(".pdfViewer .page").append("<div class='watermark'>"+pname+"</div>");


      $(".toolbar").hide();
      console.log($(".page").size() + "," + $(".page").eq(0).height());
      var canmode=0;
      setTimeout(function(){
          heigtest();
          if(pname == ''){
          ...
          $(".pdfViewer .page").each(function () {
              $(this).append("<div class='watermark'>"+pname+"</div>");
          });
      },500);
      function heigtest(){
        if(canmode<3){
            var size=$(".page").size();
            var pagehei=$(".page").eq(0).height();
            var pageheight=$("#viewer").height()+50;
            $("#Pdfbox", window.parent.document).height(pageheight);
            $("#viewerContainer").height(pageheight);
            if($("#viewer").height()>pageheight){
                canmode++;
            }
            setTimeout(function(){heigtest();},500);
        }
